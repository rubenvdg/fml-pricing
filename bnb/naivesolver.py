...
class NaiveSolver(BranchAndBound):
    ...
    def gradient(self, p):
        ...
        return np.sum(
            (prob * w_ * (1 - p_ * b_ + b_ * np.sum(prob * p_, axis=1, keepdims=True))),
            axis=0,
        )
    ...
    def get_lhs_vee(q: np.ndarray, p: np.ndarray, b: np.ndarray) -> np.ndarray:
        ...
        """
        p_q_ub = np.repeat(np.expand_dims(q * p, axis=1), len(p), axis=1)
    ...
    def _get_lipschitz_constant(lhs_vee, rhs_vee, q_ub_r, b, w):
        return np.max(
            np.sum(
                np.expand_dims(w, axis=-1) * q_ub_r * np.maximum(rhs_vee, lhs_vee),
                axis=0,
            )
            * b
        )
